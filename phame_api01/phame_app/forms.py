...
class PhameInputForm(ModelForm):
    ...
    buildSNPdb = forms.BooleanField(initial=1, label='build SNP database for all complete genomes', required=False)
    first_time = forms.BooleanField(initial=1, label='existing SNP alignment?', required=False)
    ...
    bootstrap = forms.BooleanField(initial=1, required=False)
    ...
    clean = forms.BooleanField(initial=1, required=False)
