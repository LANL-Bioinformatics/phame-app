...
def insert_user_projects(proj_dir, user):
    for project_name in os.listdir(proj_dir):
        ...
        if not os.path.exists(os.path.join(
            os.path.join(proj_dir, project_name, 'workdir', 'results'),
            f'{project_name}.log')):
            ...
            mod_time = os.path.getmtime(
                os.path.join(proj_dir, project_name, 'config.ctl'))
        ...
        project = Project(name=project_name, user=user,
                          num_threads=num_threads, execution_time=exec_time,
                          status=project_task_status, start_time=start_time,
                          end_time=end_time)
...
def insert_all_projects():
    """
    Initialize projects database by traversing users' project directories to
    get project parameters
    """
    users = User.query.all()
    for user in users:
        proj_dir = os.path.join(app.config['PROJECT_DIRECTORY'], user.username)
        insert_user_projects(proj_dir, user)


@cli.command()
def insert_public_projects():
    """
    Add public projects
    :return:
    """
    user = User.query.filter_by(username='public').first()
    proj_dir = os.path.join(app.config['PROJECT_DIRECTORY'], 'public')
    for project_name in os.listdir(app.config['PUBLIC_PROJECTS_DIR']):
        shutil.copytree(os.path.join(app.config['PUBLIC_PROJECTS_DIR'], project_name),
                    os.path.join(proj_dir, project_name))
    insert_user_projects(proj_dir, user)
